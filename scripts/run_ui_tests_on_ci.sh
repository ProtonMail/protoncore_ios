...
echo "xcodebuild -workspace \"ExampleApp.xcworkspace\" -scheme \"$SCHEME\" -destination \"$DESTINATION\" -resultBundlePath \"$UI_TESTS_RESULTS\" -derivedDataPath \"$DERIVED_DATA_PATH\" -parallel-testing-enabled $PARALLEL_TESTING_ENABLED -parallel-testing-worker-count $PARALLEL_TESTING_WORKER_COUNT -quiet test DYNAMIC_DOMAIN=$DYNAMIC_DOMAIN"

xcodebuild -workspace "ExampleApp.xcworkspace" -scheme "$SCHEME" -destination "$DESTINATION" -resultBundlePath "$UI_TESTS_RESULTS" -derivedDataPath "$DERIVED_DATA_PATH" -parallel-testing-enabled "$PARALLEL_TESTING_ENABLED" -parallel-testing-worker-count "$PARALLEL_TESTING_WORKER_COUNT" -quiet test DYNAMIC_DOMAIN=$DYNAMIC_DOMAIN
