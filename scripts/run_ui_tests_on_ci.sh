...
if [ "$1" = "--regression" ]; then 
    TESTPLAN="Example-UITests-regression-V5"
    PARALLEL_TESTING_ENABLED="NO"
    PARALLEL_TESTING_WORKER_COUNT="1"
elif [ "$1" = "--smoke" ]; then 
    TESTPLAN="Example-UITests-smoke-V5"
...
echo "xcodebuild -workspace \"example-app/example-app-v5/ExampleApp-V5.xcworkspace\" -scheme \"Example-UITests-V5\" -testPlan $TESTPLAN -destination $DESTINATION -resultBundlePath $UI_TESTS_RESULTS -derivedDataPath $DERIVED_DATA_PATH -parallel-testing-enabled $PARALLEL_TESTING_ENABLED -parallel-testing-worker-count $PARALLEL_TESTING_WORKER_COUNT -quiet test DYNAMIC_DOMAIN=$DYNAMIC_DOMAIN"

xcodebuild -workspace "example-app/example-app-v5/ExampleApp-V5.xcworkspace" -scheme "Example-UITests-V5" -testPlan "$TESTPLAN" -destination "$DESTINATION" -resultBundlePath "$UI_TESTS_RESULTS" -derivedDataPath "$DERIVED_DATA_PATH" -parallel-testing-enabled "$PARALLEL_TESTING_ENABLED" -parallel-testing-worker-count "$PARALLEL_TESTING_WORKER_COUNT" -quiet test DYNAMIC_DOMAIN="$DYNAMIC_DOMAIN"
