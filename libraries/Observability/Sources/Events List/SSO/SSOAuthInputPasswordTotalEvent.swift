...
public enum SSOAuthInputPasswordCodeStatus: String, Encodable, CaseIterable {
    case unlockSuccess
    case noKeySalt
    case noPrimaryKey
...
public struct SSOAuthInputPasswordTotalLabels: Encodable, Equatable {
    let status: SSOAuthInputPasswordCodeStatus
...
extension ObservabilityEvent where Payload == PayloadWithLabels<SSOAuthInputPasswordTotalLabels> {
    public static func ssoAuthInputPassword(status: SSOAuthInputPasswordCodeStatus) -> Self {
        .init(name: "ios_core_auth_sso_inputPassword_total",
