//
//  ExternalAccountCreationSigninEvent.swift
...
extension ObservabilityEvent where Payload == CounterPayloadWithLabels<ExternalAccountCreationLabels> {
    public static func externalAccountCreationSignin(status: SuccessOrFailureStatus) -> Self {
        .init(name: "ios_core_external_account_creation_signin", labels: .init(status: status))
    }
}
