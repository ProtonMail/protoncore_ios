...
public struct PreviewsData {
#if DEBUG
    static let remoteManager = MockRemoteManager()
    ...
    static let currentSub = CurrentSubscriptionResponse(
        ...
        entitlements: [
            ...
            .description(
                DescriptionEntitlement(type: "description", text: "Highest VPN speed", iconName: "rocket")
            ),
            .description(
                DescriptionEntitlement(type: "description", text: "5 users", iconName: "user")
            ),
            .progress(
                ProgressEntitlement(title: "Mail storage", type: "progress", text: "0.6 GB of 1 GB", min: 0, max: 10, current: 6)
            )
        ],
decorations: [])
