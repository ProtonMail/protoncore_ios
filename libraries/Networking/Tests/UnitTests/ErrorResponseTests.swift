...
class ErrorResponseTests: XCTestCase {
    ...
    func testResponseErrorHttpErrorCodeIsAddedOnlyOnceForNestedResponseErrors() {
        let deepestError = ResponseError(httpCode: 401, responseCode: 123, userFacingMessage: nil, underlyingError: nil)
        let underlyingError = ResponseError(httpCode: 402, responseCode: 123, userFacingMessage: nil,
                                            underlyingError: deepestError as NSError)
        let topMostError = ResponseError(httpCode: 403, responseCode: 123, userFacingMessage: nil,
                                         underlyingError: underlyingError as NSError)
        XCTAssertEqual(topMostError.httpCode, 403)
        XCTAssertEqual(topMostError.responseCode, 123)
        XCTAssertEqual(topMostError.localizedDescription, "Network error (http code 401)")
    }
}
