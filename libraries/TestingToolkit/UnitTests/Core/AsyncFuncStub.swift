...
@propertyWrapper
public final class AsyncFuncStub<Input, Output, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12, A13> {

    public var wrappedValue: AsyncStubbedFunction<Input, Output, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12, A13>

    init(initialReturn: @escaping (Input) throws -> Output, function: String, line: UInt, file: String) {
        wrappedValue = AsyncStubbedFunction(initialReturn: .init(initialReturn), function: function, line: line, file: file)
    }

    init(initialReturn: InitialReturn<Input, Output>, function: String, line: UInt, file: String) {
        wrappedValue = AsyncStubbedFunction(initialReturn: initialReturn, function: function, line: line, file: file)
    }

    init(function: String, line: UInt, file: String) where Output == Void {
        wrappedValue = AsyncStubbedFunction(initialReturn: .init { _ in }, function: function, line: line, file: file)
    }
}

public final class AsyncStubbedFunction<Input, Output, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12, A13> {
    ...
    private lazy var implementation: (UInt, Input) async -> Output = { [unowned self] _, input in
    ...
    func replaceBody(_ newImplementation: @escaping (UInt, Input) async -> Output) {
        initialReturn = nil
        implementation = newImplementation
    }

    func appendBody(_ additionalImplementation: @escaping (UInt, Input) async -> Output) {
        ...
        implementation = {
            ...
            _ = await currentImplementation($0, $1)
            return await additionalImplementation($0, $1)
        }
    }

    func callAsFunction(input: Input, arguments: CapturedArguments<Input, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12, A13>) async -> Output {
        capturedArgumentsAndCounterStorage.mutate {
            $0.0 += 1
            $0.1.append(arguments)
        }
        return await implementation(callCounter, input)
    }
