...
final class V4SubscriptionRequestTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        sut = V4SubscriptionRequest(api: APIServiceMock(), planName: "testName", amount: 123, currencyCode: "EUR", cycle: 12, paymentAction: .token(token: "token"))
    }

    func test_tokenParameters() {
        // Given
        let token = "thisIsAToken"
        sut = V4SubscriptionRequest(api: APIServiceMock(), planName: "testName", amount: 123, currencyCode: "EUR", cycle: 13, paymentAction: .token(token: token))
    ...
    func test_appleParameters() {
        // Given
        sut = V4SubscriptionRequest(api: APIServiceMock(), planName: "testName", amount: 123, currencyCode: "CHF", cycle: 11, paymentAction: .apple(receipt: "receipt"))
