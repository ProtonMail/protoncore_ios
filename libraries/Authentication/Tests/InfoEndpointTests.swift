...
final class InfoEndpointTests: XCTestCase {
    ...
    let externalSignupHeader = Feature.init(name: "externalSignupHeader", isEnable: true, flags: [.availableCoreInternal])
    ...
    func testHeaderWithFeatureSwitch() {
        withFeatureSwitches([externalSignupHeader]) {
            XCTAssertEqual(["X-Accept-ExtAcc": true], cut.header as! [String: Bool])
        }
    }

    func testHeaderWithoutFeatureSwitch() {
        withFeatureSwitches([]) {
            XCTAssertEqual([:], cut.header as! [String: Bool])
        }
    }
}
