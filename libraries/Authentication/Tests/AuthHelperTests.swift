...
class AuthHelperTests: XCTestCase {
    ...
    /// * that doesn't refresh if no credentials
    
    func testAuthHelperDoesNotRefreshIfNoCredentials() async throws {
        let apiService = APIServiceMock()
        let out = AuthHelper()
        let result = await withCheckedContinuation { continuation in
            out.onRefresh(sessionUID: "test session", service: apiService, complete: continuation.resume(returning:))
        }
        guard case .failure = result else { XCTFail(); return }
    }
    
    /// * that doesn't refresh if wrong session
    
    func testAuthHelperDoesNotRefreshIfWrongSession() async throws {
        let apiService = APIServiceMock()
        let out = try XCTUnwrap(AuthHelper(initialBothCredentials: (initialAuthCredential, initialCredential)))
        let result = await withCheckedContinuation { continuation in
            out.onRefresh(sessionUID: "wrong session", service: apiService, complete: continuation.resume(returning:))
        }
        guard case .failure = result else { XCTFail(); return }
    }
    
    /// * that calls refresh endpoint using provided api service
    ...
    ///   * that handles updatedCredential
    ...
    func testAuthHelperPassesRefreshError() async throws {
        let apiService = APIServiceMock()
        ...
        XCTAssertTrue(apiService.requestDecodableStub.wasCalledExactlyOnce)
        guard case .failure(.networkingError(let responseError)) = result else { XCTFail(); return }
