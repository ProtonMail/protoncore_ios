...
class PMAuthenticationTests: XCTestCase {
            ...
            case .success(let stage):
                ...
                manager.confirm2FA("111111", context: context) { result in
                    ...
                    guard case Result.failure(let serverError) = result,
                          case Authenticator.Errors.networkingError(let errorResponse) = serverError else
                    {
    ...
    // temporarily disabled due to the BE anti abuse protection - Too many recent failed login attempts
