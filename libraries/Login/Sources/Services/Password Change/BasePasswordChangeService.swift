//
//  BasePasswordChangeService.swift
//  ProtonCore-Login - Created on 10.01.2025.
//
...
open class BasePasswordChangeService {
    ...
    public func updateLoginPassword(auth currentAuth: AuthCredential,
        ...
        let info = try await fetchAuthInfo()
    ...
    public func updateUserPassword(auth currentAuth: AuthCredential,
                                   ...
                                   buildAuth: Bool,
                                   skipPasswordSRPProof: Bool = false,
        ...
        var srpAuthValues: UpdatePrivateKeyRequest.SRPAuthValues?
        if !skipPasswordSRPProof {
            srpAuthValues = try await calculatePasswordSRPProof(
                username: username,
        ...
        if let deviceSecret {
            encryptedSecret = try GetEncryptedSecret().invoke(
                passphrase: resultOfKeyUpdate.hashedNewPassword.value,
                deviceSecret: deviceSecret
            )
        }

        let updatePrivateKeyRequest = UpdatePrivateKeyRequest(
            srpAuthValues: srpAuthValues,
    ...
    public func fetchAuthInfo() async throws -> AuthInfoResponse {
        return try await withCheckedThrowingContinuation { continuation in
            authService.info(intent: .proton) { response in
                switch response {
    ...
    private func calculatePasswordSRPProof(
        username: String,
        loginPassword: String,
        authInfoResponse: AuthInfoResponse?
    ) async throws -> UpdatePrivateKeyRequest.SRPAuthValues {

        let authInfo = authInfoResponse != nil ? authInfoResponse! : try await fetchAuthInfo()
        ...
        return UpdatePrivateKeyRequest.SRPAuthValues(
