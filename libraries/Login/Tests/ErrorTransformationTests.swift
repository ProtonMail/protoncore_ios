...
final class ErrorTransformationTests: XCTestCase {
    ...
    let apiBlockedError = ResponseError(httpCode: nil, responseCode: nil, userFacingMessage: nil,
                                        underlyingError: NSError.protonMailError(APIErrorCode.potentiallyBlocked,
    ...
    func testAuthErrorToAvailabilityErrorTransformation() {
        ...
        let notAvailableError = ResponseError(httpCode: 401, responseCode: 12106, userFacingMessage: "test not available error", underlyingError: nil)
        XCTAssertEqual(
