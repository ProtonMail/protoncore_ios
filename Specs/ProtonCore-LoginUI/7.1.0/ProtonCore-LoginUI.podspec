...
Pod::Spec.new do |s|
    ...
    make_subspec = ->(spec, crypto) {
        spec.subspec "#{crypto_subspec(crypto)}" do |subspec|
            ...
            subspec.dependency "ProtonCore-HumanVerification/#{crypto_subspec(crypto)}", $version
    ...
    make_unit_test_subspec = ->(spec, crypto) {
        spec.test_spec "Unit#{crypto_test_subspec(crypto)}" do |test_spec|
            test_spec.dependency "ProtonCore-Crypto/#{crypto_subspec(crypto)}", $version
            ...
            test_spec.dependency "ProtonCore-HumanVerification/#{crypto_subspec(crypto)}", $version
